...
import { Search as SearchIcon, Rss, Plus, Check, Play, X } from 'lucide-react'
...
export default function Search() {
  ...
  const { addSubscription, subscriptions, recentFeeds, blacklist, addToBlacklist, isBlacklisted } = useRSSStore()
  ...
  useEffect(() => {
    // 加载热门播客作为默认结果，并从缓存中获取图片信息，过滤黑名单
    const popularFeeds = getPopularRSSFeeds().filter(feed => !blacklist.includes(feed.rssUrl))
    setResults(enrichFeedsWithCache(popularFeeds))
  }, [recentFeeds, blacklist])
  ...
  const handleSearch = async (searchKeyword = keyword) => {
    ...
    try {
      ...
      const filteredResults = searchResults.filter(feed => !blacklist.includes(feed.rssUrl))
      const results = filteredResults.length > 0 ? filteredResults : getPopularRSSFeeds().filter(feed => !blacklist.includes(feed.rssUrl))
