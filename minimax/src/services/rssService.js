...
export const parseRSS = async (url, forceRefresh = false) => {
  ...
  if (CONFIG.USE_GO_BACKEND) {
    return await parseRSSWithGo(url, forceRefresh)
  }
 ...
 * 使用 Go 后端解析 RSS
 */
const parseRSSWithGo = async (url, forceRefresh = false) => {
  try {
    if (import.meta.env.DEV) {
      console.log(`[Go后端] 正在请求: ${CONFIG.GO_BACKEND_URL}/api/feed?url=${encodeURIComponent(url)}`)
    }
    
    const response = await fetch(`${CONFIG.GO_BACKEND_URL}/api/feed?url=${encodeURIComponent(url)}`, {
      method: 'GET',
      headers: {
    ...
    const result = {
      ...
      episodes: (feedData.items || []).map(item => ({
        ...
        image: item.image || feedData.image,
    ...
    if (import.meta.env.DEV) {
      console.error(`[Go后端] 解析失败: ${url}`, error)
    }
    ...
    const staleCache = cache[url]
    if (staleCache?.data) {
      console.warn(`⚠️ Go 后端解析失败，使用缓存数据: ${url}`)
      return staleCache.data
    }
